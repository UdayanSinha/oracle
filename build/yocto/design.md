...
- Poky is Yocto's reference distribution config., and that is used as the baseline.
    - `meta-skeleton` is a layer in Poky containing official examples.
...
1. Important variables to modify under `build/conf/local.conf` (specified for `CONF_VERSION` = 2):
    ...
    - `PACKAGE_CLASSES` : Packaging format (RPM, DEB, etc) to use for built packages.
        - The rootfs will ultimately be generated from these packages.
...
1. Create a new layer: `bitbake-layers create-layer /path/to/meta-<layer-name>`
    ...
    - Layer priority is set within `/path/to/meta-<layer-name>/conf/layer.conf` using `BBFILE_PRIORITY_<layer-name>` variable.
...
7. Use development shell (devshell) to aid in recipe debugging, if needed: `bitbake -c devshell <recipe-name>`
    ...
    - If running on a remote system via SSH, it is recommended to have tmux/screen installed in that system.
...
5. To export recipe to a layer (i.e. a layer in `BBLAYERS` other than workspace layer): `devtool finish <recipe-name> /path/to/meta-<layer-name>`
    ...
    - If the recipe exists in a layer (`devtool modify ...`):
        ...
        - A typical patch-creation workflow would look like this:
...
2. `extrausers` class:
    ...
    - E.g. shows how to add a new user and a password to root user:
        ...
        # see: https://man7.org/linux/man-pages/man8/usermod.8.html
        EXTRA_USERS_PARAMS = "\
