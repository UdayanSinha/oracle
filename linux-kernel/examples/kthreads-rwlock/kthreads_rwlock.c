/*
 * kthreads_rwlock.c: Example of kthreads using rwlock_t
 */
...
#include <linux/rwlock.h>	/* rwlock APIs */
...
static rwlock_t lock;			/* see DEFINE_RWLOCK() as well */
...
static int kt1_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		/* access shared resource with non-blocking read lock */
...
static int kt2_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		/* access shared resource with read lock */
		read_lock(&lock);
...
static int __init kthreads_rwlock_init(void)
{
	int ret = 0;
...
static void __exit kthreads_rwlock_exit(void)
{
	int ret = 0;
...
module_init(kthreads_rwlock_init);
module_exit(kthreads_rwlock_exit);

MODULE_AUTHOR("Udayan Prabir Sinha");
MODULE_LICENSE("Dual MIT/GPL");
MODULE_DESCRIPTION("Rwlock example with kthreads");
