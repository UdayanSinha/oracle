/*
 * kthreads_mutex.c: Example of kthreads using mutex
 */
...
#include <linux/mutex.h>	/* mutex APIs */
...
static int __init kthreads_mutex_init(void)
{
	int ret = 0;
...
static void __exit kthreads_mutex_exit(void)
{
	int ret = 0;
...
module_init(kthreads_mutex_init);
module_exit(kthreads_mutex_exit);
