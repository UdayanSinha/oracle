...
5) Create a QEMU disk image and run QEMU:
	...
	* sudo debootstrap --variant=minbase --merged-usr stable ${MNT} http://deb.debian.org/debian
...
6) If network interfaces are needed for the QEMU target (see https://wiki.qemu.org/Documentation/Networking):
	a) Run QEMU (host port 5555 forwarded to target port 22 for SSH access):
		* sudo qemu-system-x86_64 -enable-kvm -cpu ${CPU} -smp ${SMP} -m ${RAM} -nographic -machine q35 -kernel /path/to/bzImage -hda /path/to/disk.img -append "root=/dev/sda rw console=ttyS0" --device e1000,netdev=net0 -netdev user,id=net0,hostfwd=tcp::5555-:22
	b) [Optional] If the target network interface is not brought up automatically, then bring up the interface, and assign an IP address using DHCP:
		* ip link set enp0s2 up		--> run on target
		* dhclient enp0s2		--> run on target
	c) [Optional] The target network interface may need to be in promiscous mode with certain kernel builds to ensure consistent communication:
